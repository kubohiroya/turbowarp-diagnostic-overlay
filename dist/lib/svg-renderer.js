...
export function renderDiagnosticSvg(input, options = {}) {
    ...
    const parts = [];
    parts.push(`<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${resolved.width} ${resolved.height}" role="img" aria-labelledby="diagnostic-title diagnostic-description">`, `<title id="diagnostic-title">${escapeXml(`${theme.label} ${diagnostic.code}`)}</title>`, `<desc id="diagnostic-description">${escapeXml(accessibleDiagnosticText(diagnostic))}</desc>`, `<rect width="${resolved.width}" height="${resolved.height}" fill="#05070A" fill-opacity="0.42"/>`, `<rect x="${panelX}" y="${panelY}" width="${panelWidth}" height="${panelHeight}" rx="12" fill="#151922" fill-opacity="0.97" stroke="${theme.accent}" stroke-width="2"/>`, `<rect x="${panelX}" y="${panelY}" width="6" height="${panelHeight}" rx="3" fill="${theme.accent}"/>`, textElement([`${theme.label}  ${diagnostic.code}`], textX, cursorY, titleSize, theme.accent, 700));
    cursorY += Math.round(messageSize * 1.7);
    const messageLines = truncateLines(wrapText(diagnostic.message, maxUnits(textWidth, messageSize)), resolved.maxMessageLines, maxUnits(textWidth, messageSize));
    ...
    if (diagnostic.detail) {
        const detailLines = truncateLines(wrapText(diagnostic.detail, maxUnits(textWidth, bodySize)), resolved.maxDetailLines, maxUnits(textWidth, bodySize));
    ...
    if (diagnostic.source?.excerpt) {
        ...
        const excerptLines = truncateLines(wrapText(diagnostic.source.excerpt, maxUnits(excerptWidth, smallSize)), excerptLineLimit, maxUnits(excerptWidth, smallSize));
        const excerptHeight = lineBlockHeight(excerptLines.length, smallSize) + excerptPadding * 2;
        parts.push(`<rect x="${textX}" y="${excerptY}" width="${textWidth}" height="${excerptHeight}" rx="5" fill="#090C12" stroke="#303848"/>`, textElement(excerptLines, textX + excerptPadding, excerptY + excerptPadding + smallSize, smallSize, '#E7ECF5', 400, 'ui-monospace, SFMono-Regular, Menlo, Consolas, monospace'));
    }
    parts.push('</svg>');
    return parts.join('');
}
export function accessibleDiagnosticText(diagnostic) {
    return [
